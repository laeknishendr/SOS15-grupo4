...
public class EmisionesServlet extends HttpServlet {
	...
	public void process(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException {
		/*
		 * Este método, que utilizaremos como método por defecto para todos
		 * los comandos, tendremos que pillar el contenido de la URL, el método
		...
		if (path != null) {
			...
			/*
			 * En caso de que el path no sea nulo, accedemos al recurso en
			 * concreto. Aquí podremos hacer GET, POST y DELETE.
			 */
			...
			processResource(req, resp, resource, method);
	...
	/*
	 * métodos CRUD relacionados con la obtención de la lista de elementos del
	 * dataset
	 * 
	 * - GET - POST - DELETE
	 */
	...
	public void processResourceList(HttpServletRequest req, HttpServletResponse resp, String method) throws IOException {
		switch (method) {

		case "GET":
			getEmissions(req, resp);
			System.out.println("Llega hasta el ProcessResourceList");
			break;

		case "PUT":
			resp.sendError(resp.SC_METHOD_NOT_ALLOWED, "METHOD_NOT_ALLOWED");
			break;

		case "POST":
			postEmissions(req, resp);
			break;
	...
	public void getEmissions(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		while (it.hasNext()) {
			...
			Entity e = it.next();
			Emission em = new Emission((String) e.getProperty("country"), (Double) e.getProperty("CO2emissions"), (Long) e.getProperty("population"), (Long) e.getProperty("year"));
	...
	/*
	 * métodos CRUD para tratamiento de elementos concretos del dataset - GET -
	 * PUT - DELETE
	 */

	@SuppressWarnings("static-access")
	public void processResource(HttpServletRequest req, HttpServletResponse resp, String resource, String method) throws IOException {
		...
		case "GET":
			getEmission(req, resp, e);
			break;

		// getEmission(req, resp, resource); break;

		case "PUT":
			updateEmission(req, resp, resource);
			break;
		...
		case "DELETE":
			removeResource(req, resp, e);
			break;

		}
	}

	private void getEmission(HttpServletRequest req, HttpServletResponse resp, Entity e) throws IOException {
	...
	@SuppressWarnings("static-access")
	private void updateEmission(HttpServletRequest req, HttpServletResponse resp, String resource) throws IOException {
