...
public class ConsumosEnergiasServlet extends HttpServlet {
	...
	@SuppressWarnings({ "static-access" })
	private void postConsumos(HttpServletRequest req, HttpServletResponse resp) throws IOException {
	...
	private void processResource(String method, String resource, HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		case "DELETE":
			removeResource(req, resp, e); // datastore.delete(extractEntity(req).getKey());
											// break;
		}
	...
	@SuppressWarnings("static-access")
	private void updateConsumos(String resource, HttpServletRequest req, HttpServletResponse resp) throws IOException {

		Entity e = extractEntity(req);

		if (e == null) {
			resp.setStatus(resp.SC_BAD_REQUEST);
		} else if (!(e.getProperty("country").equals(resource))) {
