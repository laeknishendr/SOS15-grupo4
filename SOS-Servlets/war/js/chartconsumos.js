...
function dibujar() {
	...
	request.done(function(dataR, status, jqXHR) {
		...
		for (count = 0; count < jsonArray.length; count++) {
			...
			var energy_use = parseFloat(JSON
					.stringify(jsonArray[count].energy_use));
			data.addRows([ [ country, energy_use ] ]);
		...
		var grafica = new google.visualization.GeoChart(document
