google.load('visualization','1.0',{'packages':['corechart']});
		...
		{ 
			var request = $.ajax({
				url: "/api/v1/emisiones", //http://sos-2015-04.appspot.com
				type: "GET",
				dataR: $("#payload").val()
			});
			...
			request.done(function(dataR, status, jqXHR){
				...
				for (count=0;count<jsonArray.length;count++){
					...
					var CO2emissions = parseFloat(JSON.stringify(jsonArray[count].CO2emissions));
					data.addRows(
						[
				...
				var opciones = {'title':'CO2emissions in country',
								'width': 800,
								'height':600};
